...
const DASHBOARD_PROJECT_ID = '82981f9f-88d6-4df9-9dfc-1450fdddc9cc'
const SANDBOX = {
...
export default defineRailway((ctx) => {
  // A whole-project apply deletes what the file leaves out (in Sandboxes, every
  // instance), so the file only ever targets Dashboard.
  if (ctx.projectId !== DASHBOARD_PROJECT_ID) {
    throw new Error(
      'This file only manages the Dashboard project: railway link --project Dashboard',
    )
  }
